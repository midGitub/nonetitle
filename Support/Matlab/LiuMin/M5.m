...
WildMultiplier=zeros(NumOfItem,1);%每个组合因Wild所带来的倍数
for i=1:3 
    %Ordered3
    WildMultiplier(i)=1;
...
for i=4:8
    ...
    %以下区域为每台机器不同的地方%
    R11=Reel1WildNum(i,1);R12=Reel1WildNum(i,2);R13=Reel1WildNum(i,3);%%为方便下面的计算，所以简化变量名
    R1=R11+R12+R13;
    ...
    WildMultipleInAll3=WildMultipleInAll3+P001*(R3*5)/R3;%添加001带来的基础倍数
    WildMultipleInAll3=WildMultipleInAll3+P010*(R21*15+R22*10+R23*5)/R2;%添加010带来的基础倍数
    WildMultipleInAll3=WildMultipleInAll3+P011*(R21*15+R22*10+R23*5)/R2*(R3*5)/R3;%添加011带来的基础倍数
    WildMultipleInAll3=WildMultipleInAll3+P100*(R1*5)/R1;%添加100带来的基础倍数
    WildMultipleInAll3=WildMultipleInAll3+P101*(R1*5)/R1*(R3*5)/R3;%添加101带来的基础倍数
    WildMultipleInAll3=WildMultipleInAll3+P110*(R1*5)/R1*(R21*15+R22*10+R23*5)/R2;%添加110带来的基础倍数
    %以上区域为每台机器不同的地方%
...
for i=9 
    ...
    WildMultipleInAny3Symbols2=WildMultipleInAny3Symbols2+P100*(R1*5)/R1;%添加100带来的基础倍数，即第一个卷轴的wild带来的基础倍数
    WildMultipleInAny3Symbols2=WildMultipleInAny3Symbols2+P010*(R21*15+R22*10+R23*5)/R2;%添加010带来的基础倍数，即第二个卷轴的wild带来的基础倍数
    WildMultipleInAny3Symbols2=WildMultipleInAny3Symbols2+P001*(R3*5)/R3;%添加001带来的基础倍数，即第三个卷轴的wild带来的基础倍数
    %以上区域为每台机器不同的地方%
...
for i=10 
    ...
    %按是否可替换为wild来分类（110，101，011,111）=（7/27,7/27,7/27,2/9）.1可以替换为wild. 
    WildPossibility110=7/27;%第一、二卷轴的symbol相同的概率（只有第一、二卷轴有替换为wild的可能性）
    WildPossibility101=7/27;%第一、三卷轴的symbol相同的概率（只有第一、三卷轴有替换为wild的可能性）
    WildPossibility011=7/27;%第二、三卷轴的symbol相同的概率（只有第二、三卷轴有替换为wild的可能性）
    WildPossibility111=2/9;%第一、二、三卷轴的symbol都不相同的概率（第一、二、三卷轴都有可能替换为wild）
    %以下区域为每台机器不同的地方%
    R11=Reel1WildNum(i,1);R12=Reel1WildNum(i,2);R13=Reel1WildNum(i,3);
    R1=R11+R12+R13;
    R21=Reel2WildNum(i,1);R22=Reel2WildNum(i,2);R23=Reel2WildNum(i,3);
    ...
    StandardHit(i)=P000;
    %以下区域为每台机器不同的地方%
    WildMultipleInAny3Symbols3=0;%因为Wild所带来的倍数，包含StandardHit的情况
    WildMultipleInAny3Symbols3=WildMultipleInAny3Symbols3+P000*1;%添加000带来的基础倍数，即StandardHit的基础倍数
    WildMultipleInAny3Symbols3=WildMultipleInAny3Symbols3+P100*(R1*5)/R1;%添加100带来的基础倍数，即第一个卷轴的wild带来的基础倍数
    WildMultipleInAny3Symbols3=WildMultipleInAny3Symbols3+P010*(R21*15+R22*10+R23*5)/R2;%添加010带来的基础倍数，即第二个卷轴的wild带来的基础倍数
    WildMultipleInAny3Symbols3=WildMultipleInAny3Symbols3+P001*(R3*5)/R3;%添加001带来的基础倍数，即第三个卷轴的wild带来的基础倍数
    %以上区域为每台机器不同的地方%
