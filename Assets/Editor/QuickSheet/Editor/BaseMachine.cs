...
namespace UnityQuickSheet
{
	...
	{
		[SerializeField]
		public List<ColumnHeader> list = new List<ColumnHeader>();

		[SerializeField]
		protected bool isReuseOtherSheet = false;
		public bool IsReuseOtherSheet
		{
			get { return isReuseOtherSheet; }
			set { isReuseOtherSheet = value; }
		}

		[SerializeField]
		protected string reuseSheetName = string.Empty;
		public string ReuseSheetName
		{
			get { return reuseSheetName; }
			set { reuseSheetName = value; }
		}

		[SerializeField]
		public bool onlyCreatePostProcessClass = false;
	}

	[System.Serializable]
	public class SerializableColumnHeaderDict : SerializableDict<string, ColumnHeaderList>
	{
	}
    ...
    public class BaseMachine : ScriptableObject
    {
        ...
        [SerializeField]
		private string templatePath = "Editor/QuickSheet/Templates";
		...
		public SerializableColumnHeaderDict ColumnHeaderListDict
        {
			get { return columnHeaderListDict; }
			set { columnHeaderListDict = value; }
        }

        [SerializeField]
		protected SerializableColumnHeaderDict columnHeaderListDict = new SerializableColumnHeaderDict();

        /// <summary>
        /// Return true, if the list is instantiated and has any its item more than one.
        /// </summary>
        /// <returns></returns>
		public bool HasColumnHeader(string name)
        {
			if(columnHeaderListDict.ContainsKey(name)
				&& columnHeaderListDict.GetValue(name) != null && columnHeaderListDict.GetValue(name).list.Count > 0)
                return true;
...
//            onlyCreatePostProcessClass = false;
        }

		public ColumnHeaderList GetCurrentWorkSheetConfig()
		{
			return columnHeaderListDict.GetValue(workSheetName);
		}

		public string GetComposedWorkSheetClassName()
		{
			ColumnHeaderList singleSheet = GetCurrentWorkSheetConfig();
			string sheetName = singleSheet.IsReuseOtherSheet ? singleSheet.ReuseSheetName : workSheetName;
			return sheetName + "Sheet";
		}

		public string GetComposedDataClassName()
		{
			ColumnHeaderList singleSheet = GetCurrentWorkSheetConfig();
			string sheetName = singleSheet.IsReuseOtherSheet ? singleSheet.ReuseSheetName : workSheetName;
