...
namespace UnityQuickSheet
{
    ...
    public class BaseMachineEditor : Editor
    {
        ...
        protected virtual ScriptPrescription Generate(BaseMachine m)
        {
			...
			if (m.GetCurrentWorkSheetConfig().onlyCreatePostProcessClass)
            {
				// Note by nichos: don't create PostProcessor class anymore!
				// All are handled by a single class: ExcelAssetPostprocessor
            ...
            else
            {
				...
				// Note by nichos: don't create Editor class any more!
				// It's just useless
        ...
        {
			sp.className = machine.GetComposedWorkSheetClassName();
        ...
        {
			if (m.HasColumnHeader(m.WorkSheetName))
            {
                ...
                {
					foreach (ColumnHeader header in m.ColumnHeaderListDict.GetValue(machine.WorkSheetName).list)
                    {
